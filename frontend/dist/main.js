        ...
        <div class="modal-project">
            ...
            <div style="margin-bottom: 24px;">
                ...
                <div class="project-tech">
                    ${project.technologies_used.split(',').map(tech => `<span class="tech-tag">${tech.trim()}</span>`).join('')}
                </div>
            </div>
...
async function fetchBlogPosts() {
    ...
    if (!blogPostsList || isLoadingBlogs)
        return;
    isLoadingBlogs = true;
    try {
...
function setupBlogSearch() {
    ...
    const handleSearch = debounce((searchTerm) => {
        const filtered = allBlogPosts.filter(post => post.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
...
function setupFormValidation() {
    ...
    if (!nameInput || !emailInput || !messageTextarea || !nameError || !emailError || !messageError)
        return;
...
export { fetchProjects, fetchBlogPosts, handleContactForm };
