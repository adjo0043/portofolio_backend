...
function renderHero() {
    ...
    if (heroEyebrow)
        heroEyebrow.textContent = profile.title;
    if (heroDescription)
    ...
    const contactBtn = document.querySelector('.hero-cta .btn-secondary');
...
function renderProjects() {
    ...
    for (const project of publishedProjects) {
        ...
        if (project.github_url) {
            const codeLink = el('a', 'btn btn-outline');
        ...
        if (project.live_url) {
            const liveLink = el('a', 'btn btn-primary');
...
function renderExperiences() {
    ...
    if (!container || portfolioData.experiences.length === 0)
        return;
    ...
    for (const exp of portfolioData.experiences) {
        ...
        if (exp.company_logo) {
            const logo = el('img', 'company-logo');
...
function renderCertifications() {
    ...
    if (!container || portfolioData.certifications.length === 0)
        return;
    ...
    for (const cert of portfolioData.certifications) {
        ...
        if (cert.credential_url) {
            const link = el('a', 'btn btn-outline btn-sm');
...
function renderLanguages() {
    ...
    if (!container || portfolioData.languages.length === 0)
        return;
...
function renderInterests() {
    ...
    if (!container || portfolioData.interests.length === 0)
        return;
...
function addCustomSectionToNav(section) {
    const navMenu = document.getElementById('nav-menu');
    if (!navMenu)
        return;
...
function renderFooter() {
    ...
    if (footerName)
        footerName.textContent = profile.full_name;
    if (footerTitle)
        footerTitle.textContent = profile.title;
    if (footerRight) {
        footerRight.innerHTML = '';
        // Email link
        if (profile.email) {
            const emailLink = el('a', 'footer-link');
